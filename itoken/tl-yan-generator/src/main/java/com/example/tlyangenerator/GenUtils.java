...
public class GenUtils {
    ...
    public static void generatorCode(Map<String, String> table,
        ...
        tableDTO.setComments(table.get("tableComment"));
        ...
        for (Map<String, String> column : columns) {
            ...
            //是否主键
            if ("PRI".equalsIgnoreCase(column.get("columnKey")) && tableDTO.getPk() == null) {
                tableDTO.setPk(columnDO);
            }

            columsList.add(columnDO);
        }
        tableDTO.setColumns(columsList);
        ...
        for (String template : templates) {
            ...
            try {
                //添加到zip
                zip.putNextEntry(new ZipEntry(getFileName(template, tableDTO.getClassname(), tableDTO.getClassName(), config.getString("package").substring(config.getString("package").lastIndexOf(".") + 1))));
                IOUtils.write(sw.toString(), zip, "UTF-8");
                IOUtils.closeQuietly(sw);
                zip.closeEntry();
            } catch (IOException e) {
                throw new BDException("渲染模板失败，表名：" + tableDTO.getTableName(), e);
            }
        }
    }
