 ...
 * @Author: zhangminjie
 * @LastEditTime: 2020-04-21 10:37:01
 * @FilePath: \vue-sales\src\views\nonCar\payment\paymentForwardPerson.vue
 -->
<template>
  <div class="paymentForward" id="paymentForward">
    ...
    <div v-show="!isShow">
      ...
      <van-cell-group class="info">
        ...
        <van-cell
          title="出生日期"
          :value="queryData.insuredBirthDate"
          value-class="textStyle"
        />
      ...
      >
        ...
        <van-cell title="发票抬头" :value="queryData.invoiceInfo.companyName" />
                <van-cell title="发票类型" :value="queryData.invoiceInfo.invoiceType =='3'? '增值税电子发票' : '增值税普通发票'" />

        <!-- <van-cell
          title="纳税人识别码"
          :value="queryData.invoiceInfo.taxPayerNo"
        /> -->
        <van-cell title="联系电话" :value="queryData.invoiceInfo.taxMobile" v-if="queryData.invoiceInfo.taxMobile!=''"/>
        <van-cell title="邮箱地址" :value="queryData.invoiceInfo.taxEmail" v-if="queryData.invoiceInfo.taxEmail != ''"/>
      </van-cell-group>
      <van-row type="flex" class="pageBottom">
        <van-col span="12">
          <van-button type="default" class="btn1">
            保费
            <span class="strong">{{ queryData.sumPremium }}</span
            >元
          </van-button>
        </van-col>
        <van-col span="12">
  ...
  methods: {
    ...
    initData() {
      ...
      forwardPayProductInsureInfo(obj).then(res => {
        ...
        // 投保人证件类型
        this.formulaList = res.data.formulas[0].amountInfoJson;
        this.content = res.data.content;
        let formulasArr = res.data.formulas;
        if (res.data.formulas.length > 0) {
          if (formulasArr[0].amountInfoJson) {
            let addRiskAmountJson = JSON.parse(formulasArr[0].amountInfoJson);
            let arr = [];
          ...
          getForwardUrl(params).then(
            res => {
              if (res.code === "1" && res.data) {
                ...
                //     url:
                //       this.$rootPath +
                //       "/nonCar/payment/paymentForwardPerson" +
                //       res.data,
