  ...
  <div id="signature-confirm">
    ...
    <div class="title-container">
      <div class="title-text">
        本人已阅读
        <a
          v-for="protocol in protocols.slice(0, 4)"
          :key="protocol.name"
        ...
        <a
          class="title-link-text"
          @click="goProtocols(protocols[4].link)"
          v-if="isFlag == 'P'"
        >《健康和职业告知》</a>
        <a
          class="title-link-text"
          @click="goProtocols(protocols[6].link)"
          v-else-if="isFlag=='N'"
        >《健康和职业告知》</a>
        <a
          class="title-link-text"
          v-else
          @click="goProtocols(protocols[5].link)"
        >《健康和职业告知》</a>
        内容真实有效！
      </div>
    </div>
    ...
    >
      ...
      <van-col
        class="signature-anew"
        @click="showSignaturePadAndReSign"
      >重签</van-col>
    </van-row>
    <div class="card">
...
export default {
  ...
  data: () => ({
    protocols: [
      { name: "《保险条款》", link: "personalTerms" },
      { name: "《投保须知》", link: "InsureNotice" },
      { name: "《投保人声明》", link: "InsureStatement" },
      { name: "《特别约定》", link: "Aboutpersonal" },
      { name: "《健康和职业告知》", link: "healthTell" },
      { name: "《健康和职业告知》", link: "healthTellFamily" },
      { name: "《健康和职业告知》", link: "healthTellNew" }
  ...
  created () {
    ...
    if (this.$route.query.isFlag) {
      if (this.$route.query.isFlag == "P") {
        this.isFlag = "p"
      } else if (this.$route.query.isFlag == "N") {
        this.isFlag = "N"

      }
    }
    // if (this.$route.query.productCode) {
    //   if (this.$route.query.productCode == "WTI01") {
    //     // this.protocols[0].
    //   }
    // }
  },
  ...
  methods: {
    goProtocols (a) {
      if (this.$route.query) {
        if (this.$route.query.productCode) {
          if (this.$route.query.productCode == "WTI01") {
            this.$router.push({ name: a, params: { from: 'WTI01' } });
          }
        } else {
    ...
    async sendCode () {
      // this.disabled = true;
      // this.insureNo = "TYEF20530126020000000070";
      let res = await signSendSMS({ insureNo: this.insureNo });
      //console.log(res);
      if (
        res.code == "1"
      ) {
        ...
        if (!this.$isNull(res.msg.replace(/[^0-9]/ig, ""))) {
          this.codeSign = parseInt(res.msg.replace(/[^0-9]/ig, ""));
        }
        //倒计时 TODO
      } else {
        Toast({
    ...
    async doUploadSignBook () {
      ...
      //console.log(this.signaturePadShow);
      let obj = {
      ...
      let res = await uploadSignBook(obj);
      //console.log(res);
    ...
    async getApplyPay () {
      ...
      if (res.code == "1") {
        ...
        if (res.data && res.data.insureInfo) {
          let { insureNo, payApplyNo, identityCode } = res.data.insureInfo;

          this.$router.push({
            name: "paymentOrder", //fufeikai TODO suncao
            query: {
