...
@Service
@Slf4j
public class SubMillionMedical2020ServiceRenewalOfInsuranceImpl
implements SubMillionMedical2020ServiceRenewalOfInsurance {
	@Autowired
	...
	public HttpResult subMillionMedical(StoreOracleRequestVO requestVo) throws Exception {
		...
		if (null != storeFormulas) {
			...
			// 保额
			coverage6.setSumInsured(jsonAmounts.getDouble("11#一般医疗保险责任"));
			// // 费率
			// coverage6.setPremiumRate(itemKindPre.getDouble("BF")
			// / jsonAmounts.getDouble("11#一般医疗保险责任"));
			// 单位保费
			...
			// 保额
			coverage5.setSumInsured(jsonAmounts.getDouble("12#重大疾病医疗保险责任"));
			// 费率
			// coverage5.setPremiumRate(itemKindPre.getDouble("BF")
			// / jsonAmounts.getDouble("12#重大疾病医疗保险责任"));
			// 单位保费
			...
			// 保额
			coverage1.setSumInsured(jsonAmounts.getDouble("13#公共交通意外伤害-飞机"));
			// 费率
			// coverage1.setPremiumRate(1.0 / jsonAmounts.getDouble("13#公共交通意外伤害-飞机"));
			...
			// 保额
			coverage2.setSumInsured(jsonAmounts.getDouble("14#公共交通意外伤害-轮船"));
			// 费率
			// coverage2.setPremiumRate(1.0 / jsonAmounts.getDouble("14#公共交通意外伤害-轮船"));
			// 单位保费
			...
			// 保额
			coverage3.setSumInsured(jsonAmounts.getDouble("15#公共交通意外伤害-火车（地铁、轻轨） "));
			// 费率
			// coverage3.setPremiumRate(1.0 / jsonAmounts.getDouble("15#公共交通意外伤害-火车（地铁、轻轨）
			// "));
			// 单位保费
			...
			// 保额
			coverage4.setSumInsured(jsonAmounts.getDouble("16#公共交通意外伤害 - 汽车 "));
			// 费率
			// coverage4.setPremiumRate(1.0 / jsonAmounts.getDouble("16#公共交通意外伤害 - 汽车 "));
			// 单位保费
			...
			if ("Y".equals(requestVo.getaKZEXClause())) {// 扩展恶性肿瘤特需医疗责任
				...
				// 保额
				coverage9.setSumInsured(jsonAmounts.getDouble("17#扩展恶性肿瘤特需医疗责任(与重大疾病医疗保险责任共用保额)"));
				// 费率
				// coverage9.setPremiumRate(addPremium.getDouble("扩展恶性肿瘤特需医疗责任")/jsonAmounts.getDouble("17#扩展恶性肿瘤特需医疗责任(与重大疾病医疗保险责任共用保额)"));
				// 单位保费
			...
			if ("Y".equals(requestVo.getaEXTZClause())) {// 特需医疗
				...
				// 保额
				coverage7.setSumInsured(jsonAmounts.getDouble("18#恶性肿瘤院外特种药品费用医疗责任"));
				// 费率
				// coverage7.setPremiumRate(addPremium.getDouble("恶性肿瘤院外特种药品费用医疗责任")/jsonAmounts.getDouble("18#恶性肿瘤院外特种药品费用医疗责任"));
				// 单位保费
			...
			if ("Y".equals(requestVo.getaEXYLClause())) {// 附加个人恶性肿瘤赴日医疗
				...
				// coverage8.setAvgSumInsured(jsonAmounts.getDouble("ZY"));
				// 保额
				coverage10.setSumInsured(jsonAmounts.getDouble("19#恶性肿瘤赴日医疗责任"));
				// 费率
				// coverage10.setPremiumRate(addPremium.getDouble("恶性肿瘤赴日医疗责任")/jsonAmounts.getDouble("19#恶性肿瘤赴日医疗责任"));
				// 单位保费
				// coverage8.setUnitPremium(addPremium.getDouble("ZY"));
				// 应收保费/真实保费/RealPremium,=调整保费
				// coverage10.setDuePremium(addPremium.getDouble("恶性肿瘤赴日医疗责任"));
			...
			if ("Y".equals(requestVo.getaEXZZClause())) {// 重离子医疗
				...
				// coverage8.setAvgSumInsured(jsonAmounts.getDouble("ZY"));
				// 保额
				coverage8.setSumInsured(jsonAmounts.getDouble("20#恶性肿瘤扩展质子重离子医疗责任"));
				// 费率
				// coverage8.setPremiumRate(addPremium.getDouble("恶性肿瘤扩展质子重离子医疗责任")/jsonAmounts.getDouble("20#恶性肿瘤扩展质子重离子医疗责任"));
				// 单位保费
				// coverage8.setUnitPremium(addPremium.getDouble("ZY"));
				// 应收保费/真实保费/RealPremium,=调整保费
				// coverage8.setDuePremium(addPremium.getDouble("恶性肿瘤扩展质子重离子医疗责任"));
