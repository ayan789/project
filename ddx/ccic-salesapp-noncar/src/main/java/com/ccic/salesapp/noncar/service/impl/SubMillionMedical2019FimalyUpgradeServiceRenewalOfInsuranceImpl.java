...
@Service
@Slf4j
public class SubMillionMedical2019FimalyUpgradeServiceRenewalOfInsuranceImpl
implements SubMillionMedical2019FimalyUpgradeServiceRenewalOfInsurance {
	...
	public HttpResult subMillionMedical(StoreOracleRequestVO requestVo) throws Exception {
		...
		if (null != storeFormulas) {
			...
			// 保额
			coverage5.setSumInsured(jsonAmounts.getDouble("11#一般医疗") * securityNumDou);
			...
			// 保额
			coverage6.setSumInsured(jsonAmounts.getDouble("12#恶性肿瘤或重大手术疾病医疗") * securityNumDou);
			// 应收保费/真实保费/RealPremium,=调整保费
			...
			// 责任代码
			coverage2.setKindCode("C100590");
			// 保额
			coverage2.setSumInsured(jsonAmounts.getDouble("13#公共交通意外伤害-汽车") * securityNumDou);
			// 应收保费/真实保费/RealPremium,=调整保费
			...
			// 保额
			coverage3.setSumInsured(jsonAmounts.getDouble("14#公共交通意外伤害-火车（地铁、轻轨）") * securityNumDou);
			// 应收保费/真实保费/RealPremium,=调整保费
			...
			// 保额
			coverage4.setSumInsured(jsonAmounts.getDouble("15#公共交通意外伤害-轮船") * securityNumDou);
			// 应收保费/真实保费/RealPremium,=调整保费
			...
			// 保额
			coverage1.setSumInsured(jsonAmounts.getDouble("16#公共交通意外伤害-飞机") * securityNumDou);
			// 应收保费/真实保费/RealPremium,=调整保费
			...
			if ("Y".equals(requestVo.getaCTS())) {// 特需医疗
				...
				// 保额
				coverage7.setSumInsured(jsonAmounts.getDouble("17#特定疾病特需医疗") * securityNumDou);
				// 应收保费/真实保费/RealPremium,=调整保费
			...
			if ("Y".equals(requestVo.getHeavyIonMedical())) {// 重离子医疗
				...
				// 保额
				coverage8.setSumInsured(jsonAmounts.getDouble("18#质子重离子医疗") * securityNumDou);
				// 应收保费/真实保费/RealPremium,=调整保费
