 ...
 * @Author: zhangminjie
 * @LastEditTime: 2020-06-19 21:38:19
 * @FilePath: \nonCarCongfig\src\views\nonCar\dynamicPayment\prPaymentOrder.vue
-->
<template>
  <div class="payCover">
    <!-- 头部 -->
...
export default {
  ...
  created () {
    ...
    } else if (this.$route.params.insureNo) {
      this.proposalNo = this.$route.params.insureNo
    } else if (this.$route.query.params) {
      this.proposalNo = JSON.parse(this.$route.query.params).proposalNo
    }
    PaymentInformation({ proposalNo: this.proposalNo }).then((res) => {
      if (res.code == "1") {
        let {
          applicantName,
          identityCode,
        ...
        this.orderId = id
        this.insuranceType = insureType
        if (res.data.paymentInformation.length) {
          let { customerName, indiMobile } = res.data.paymentInformation[0]
          this.insuredName = customerName
          this.insuredMobile = indiMobile
        }
      }
    })
  },
  ...
  methods: {
    back () {
      ...
      if (this.$isNull(this.insuranceType) && this.insuranceType == "1") {
        //跳转财产险
        this.$router.push({
      ...
      if (!this.paymentNo) {
        this.getApplyPay().then((request) => {
          if (request.code == "1" && request.data && request.data.insureInfo) {
            let { insureNo, payApplyNo, identityCode } = request.data.insureInfo
            this.proposalNo = insureNo
            this.paymentNo = payApplyNo
            this.checkNo = identityCode
          }
          ...
          getForwardUrl(params).then(
            (res) => {
              if (res.code === "1" && res.data) {
                this.$router.push({
                  path:
                    "/nonCar/dynamicPayment/prPaymentForward?orderId=" + this.orderId,
                })
                // window.location.href = res.data.url;
