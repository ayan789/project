...
<template>
  <div class="orderCard proInsurance">
    <!-- 下拉刷新 -->
    <van-pull-refresh
      v-model="isLoading"
      success-text="刷新成功"
          ...
          <van-card class="orderInfo" @click="redirectOrderDetailPage(item)">
            <div slot="title">
              <div class="orderInfoTop">
                ...
                <div class="right">
                  <button class="tempSave" v-if="item.status == 0">暂存</button>
                  ...
                  <button class="noPay" v-if="item.status == 6">已支付</button>
              ...
              <div class="orderInfoMid">
                ...
                <!-- <p>
                  <span>车牌号</span>
                  <span class="itemContent">{{ item.licensePlateNo }}</span>
                </p> -->
                <p
                  v-if="
                    item.status == 0 ||
            ...
            <div slot="footer" class="orderInfoBottom" @click.stop>
              <!-- 暂存（ 未下单） -->
              <div v-if="item.status == 0 && !item.insureNo" class="center2">
...
export default {
  ...
  methods: {
    onUpFresh() {
      ...
      // if (sessionStorage.getItem("typeFlag") == "0") {
      //   this.form.status = 4;
      // } else {
      //   this.form.status = sessionStorage.getItem("typeFlag");
      // }
    ...
    redirectOrderDetailPage(orderInfo) {
      // console.log(794567844, orderInfo);
            ...
            if (orderInfo.insureNo) {
              this.$router.push({
                ...
                query: {
                  ...
                  isIssueAfterPay: orderInfo.isSeeFee,
                  insuranceCategory: 2
                }
              });
            } else {
              this.$router.push({
                name: "tempSave",
                query: { orderId: orderInfo.id, insuranceCategory: 2 }
              });
            }
      ...
      if (orderInfo.orderSource == "1") {
        ...
        queryZc(obj).then(res => {
          if (res.code == "1" && res.data && res.data.storeOracleRequestVO) {
            _this.orderInfoObj = JSON.parse(
              JSON.stringify(res.data.storeOracleRequestVO)
            );
            this.setStoreData(orderInfo.orderNo);
          } else {
            this.$toast.fail(res.msg || res.message);
          }
        });
    ...
    setStoreData(orderNo) {
      ...
      let shareData = {
        productInfo: {
          ...
          sumPremium: sumPremium.toFixed(2), //保费
          formatPremium,
          isAgent: isAgent,
    ...
    //   return await deleteStoreOrder({ id: [id] });
    // },
    // 删除订单信息--新
    ...
    deleteOrderInfo(orderInfo) {
      Dialog.confirm({
        ...
        .then(() => {
          this.deleteOrder(orderInfo.id).then(res => {
            if (res.code == "1" && res.msg) {
              Toast.allowMultiple();
              Toast({
                forbidClick: true,
    ...
    twogotoPayPage(orderInfo) {
      console.log("财产险订单信息", JSON.stringify(orderInfo));
    ...
    downLoadInvoice() {
      ...
      console.log("跳转电子发票页面地址：" + localStorage.getItem("shortLink"));
      let param = {
        url: url,
        type: "0",
        titleBarTag: "1",
