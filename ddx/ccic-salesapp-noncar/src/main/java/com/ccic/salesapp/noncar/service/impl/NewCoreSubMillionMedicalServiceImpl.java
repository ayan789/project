...
@Service
@Slf4j
public class NewCoreSubMillionMedicalServiceImpl implements NewCoreSubMillionMedicalService {
	...
	public HttpResult subMillionMedical(StoreOracleRequestVO requestVo)
		...
		if (null != storeFormulas) {
			...
			JSONObject extensionJson = JSONObject.fromObject(storeFormulas
					.getExtensionJson());
			JSONObject jsonAmounts = JSONObject.fromObject(storeFormulas
					.getAmountsJson());
			JSONObject jsonPremiums = JSONObject.fromObject(storeFormulas
					.getPremiumsJson());
			...
			policyCustomer
					.setIdType(this.transCodeCommon(
							requestVo.getAppliIdentifyType(),
							WebContants.IndetifiType));// 个人/机构-证件类型
			...
			personInsured.setIdType(this.transCodeCommon(
					requestVo.getInsuredIdentifyType(),
					WebContants.IndetifiType));// 证件类型
			...
			personInsured.setPolHolderInsuredRelaCode(this
					.getPolHolderInsuredRelaCode(requestVo.getAppliIdentity()));// 投保人与被保人关系
			...
			insuredGroup.setPlanCode(requestVo.getFormulaCode()
					.substring(0, 10));
			insuredGroup.setNumberOfCopies(String.valueOf(requestVo
					.getFormulaSize()));
...
//			coverage2.setUnitPremium(Double.valueOf("1.0"));
			// 应收保费/真实保费/RealPremium,=调整保费
			coverage2.setDuePremium(Double.valueOf("1.0"));
...
//			coverage5.setAvgSumInsured(jsonAmounts.getDouble("EY"));
			...
			// 费率
			coverage5.setPremiumRate(itemKindPre.getDouble("EX")
					/ jsonAmounts.getDouble("EY"));
			// 单位保费
...
//			coverage6.setAvgSumInsured(jsonAmounts.getDouble("YB"));
			...
			// 费率
			coverage6.setPremiumRate(itemKindPre.getDouble("YB")
					/ jsonAmounts.getDouble("YB"));
			// 单位保费
...
//				coverage8.setUnitPremium(addPremium.getDouble("ZY"));
				...
				if (!"0".equals(result.getCode())) {
					if (quotePriceResponseDTO != null) {
						if (quotePriceResponseDTO.getProposalNo() != null
							...
							storeInsureInfo
									.setStatus(InsuranceApplicationStatus.ZANCUN);// 试算成功,暂存
							storeInsureInfo.setInsuranceType("0");
							storeInsureInfo.setHandlerCode(storeInfo
									.getUserCode());// 归属经办人
							storeInsureInfo.setOperatorCode(storeInfo
									.getUserCode());// 操作员代码
							storeInsureInfo.setOperatorName(storeInfo
									.getUserName());// 操作员姓名
							storeInsureInfo.setComCode(storeInfo
									.getUserComCode());// 客户经理comcode
							...
							Response acciResp = underwritService.underwriting(
	...
	public int getDay(Date date1, Date date2) {
		...
		if (date1 != null && date2 != null) {
			...
			now.setTime(date2);
			/*
			 * int nowDayOfYear = now.get(Calendar.DAY_OF_YEAR); int
			 * bornDayOfYear = born.get(Calendar.DAY_OF_YEAR);
			 * day=nowDayOfYear-bornDayOfYear;
			 */
			days = (now.getTimeInMillis() - born.getTimeInMillis())
					/ (1000 * 3600 * 24);
		}
