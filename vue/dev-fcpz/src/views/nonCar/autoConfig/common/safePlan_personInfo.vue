 ...
 * @Author: zhangminjie
 * @LastEditTime: 2020-07-02 21:24:58
 * @FilePath: \nonCarCongfig\src\views\nonCar\autoConfig\common\safePlan_personInfo.vue
-->
<template>
  <div class="safePlan_personInfo" id="safePlan_personInfo">
    <div class="personInfo">
      <form-create :rule="rule" v-model="fApi" :option="options" />
      <van-cell-group v-show="!isSharePage">
        <van-cell class="minTitle" title="渠道信息" />
        <van-cell title="是否代理">
          <div solt="default">
            <button
              :disabled="isDisabled"
...
export default {
  ...
  methods: {
              ...
              obj.children[n].on = {
                click: () => {
                  if (obj.field.indexOf("insuredIsHaveShebao") >= 0) {
                    this.changeCheckBox(n, obj.field, obj.backReasoning);
                  }
                }
              };
            }
        ...
        if (field == "insured") {
          ...
          for (let j = 0, leng = planElementList.length; j < leng; j++) {
            ...
            if (controlType == "inputNumber") {
              let { minValue, maxValue, id } = this.personInfoForm[
                i
              ].planElementList[j];
              let obj = {
                bizId,
    ...
    // select方法   isInsured是否是被保人信息
    changeSelect(code, value, backReasoning, isInsured) {
      // 更改被保人年龄做的操作
      ...
      this.$emit("updateValueAndPremium", code, value, backReasoning);
    },
    // 改变保障人数
          ...
          if (obj1[k].children && obj1[k].children.length > 0) {
            for (let n = 0; n < obj1[k].children.length; n++) {
              ...
              obj1[k].children[n].on = {
                click: () => {
                  this.changeCheckBox(n, obj1[k].field, obj1[k].backReasoning);
                }
              };
    ...
    // checkBox操作
    changeCheckBox(id, code, backReasoning) {
      let obj = this.fApi.getRule(code);
      let { children } = obj;
      for (let i = 0; i < children.length; i++) {
        children[i].className = "form_field_btn_text";
