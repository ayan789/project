 ...
 * @Author: zhangminjie
 * @LastEditTime: 2020-07-03 19:27:07
 * @FilePath: \nonCarCongfig\src\views\order\elePolicy\policyDownload.vue
 -->
<template>
          ...
          <van-cell-group class="top">
            <div class="radio">
              ...
              >
                ...
                <van-radio v-if="wealthPolicyNo" name="2">
                  {{ wealthPolicyNo }}
                </van-radio>
              </van-radio-group>
            </div>
...
export default {
  ...
  created() {
    this.$nextTick(() => {
      this.radioValue = this.businessPolicyNo;
    });
  },
  methods: {
    radioCilck() {
      ...
      if (this.radio === "0") {
        this.radioValue = this.businessPolicyNo;
        this.personInfo.policyNo = this.businessPolicyNo;
      }
      if (this.radio === "1") {
        this.radioValue = this.trafficPolicyNo;
        this.personInfo.policyNo = this.trafficPolicyNo;
      }
      if (this.radio === "2") {
        this.radioValue = this.wealthPolicyNo;
        this.personInfo.policyNo = this.wealthPolicyNo;
      }
      console.log(this.radioValue, "this.radioValue");
      this.$emit("PolicyNoVal", this.radioValue);
    },
    // 返回上一页
    backToPrePage() {
      this.$globalBack();
