...
@Service
@Slf4j
public class SubMillionMedicalFimalyUpgradeServiceImpl implements SubMillionMedicalFimalyUpgradeService {
	...
	public HttpResult subMillionMedical(StoreOracleRequestVO requestVo)
		...
		if (null != storeFormulas) {
			...
			for (AccidentalInjuryInfo insuredFamily : insureds) {
				...
				addPremiumZDJB = new BigDecimal(addPremiumZDJB.toString()).add(new BigDecimal(JSONObject.fromObject(jsonPremiums.getString(insuredFamily.getAge())).getString("ZDJB"))).doubleValue();
				addPremiumYBYL = new BigDecimal(addPremiumYBYL.toString()).add(new BigDecimal(JSONObject.fromObject(jsonPremiums.getString(insuredFamily.getAge())).getString("YBYL"))).doubleValue();
				addPremiumKZEX = addPremiumKZEX + JSONObject.fromObject(jsonPremiums.getString(insuredFamily.getAge())).getDouble("KZEX");
				addPremiumEXTZ = addPremiumEXTZ + JSONObject.fromObject(jsonPremiums.getString(insuredFamily.getAge())).getDouble("EXTZ");
				addPremiumEXYL = addPremiumEXYL + JSONObject.fromObject(jsonPremiums.getString(insuredFamily.getAge())).getDouble("EXYL");
				addPremiumEXZZ = addPremiumEXZZ + JSONObject.fromObject(jsonPremiums.getString(insuredFamily.getAge())).getDouble("EXZZ");
			}
			...
			// 保额
			coverage5.setSumInsured(jsonAmounts.getDouble("ZDJB")*securityNumDou);
			// 应收保费
			coverage5.setDuePremium(addPremiumZDJB);
			...
			// 保额
			coverage6.setSumInsured(jsonAmounts.getDouble("YBYL")*securityNumDou);
			// 应收保费
			coverage6.setDuePremium(addPremiumYBYL);
			policyCoverageList1.add(coverage6);

			/*// 责任信息7 百万医疗
			PolicyCoverage coverage7 = new PolicyCoverage();
			// 条款代码
			coverage7.setClauseCode("CF1101232");
			// 责任代码
			coverage7.setKindCode("C101091");
			// 保额
			coverage7.setSumInsured(jsonAmounts.getDouble("BWYL")*securityNumDou);
			// 应收保费
			coverage7.setDuePremium(Double.valueOf("1.0")*securityNumDou);
			...
			if ("Y".equals(requestVo.getaKZEXClause())) {//扩展恶性肿瘤特需医疗责任
				...
				// 保额
				coverage8.setSumInsured(jsonAmounts.getDouble("KZEX")*securityNumDou);
				// 应收保费
			...
			if ("Y".equals(requestVo.getaEXTZClause())) {//恶性肿瘤院外特种药品费用医疗责任
				...
				// 保额
				coverage9.setSumInsured(jsonAmounts.getDouble("EXTZ")*securityNumDou);
				// 应收保费
			...
			if ("Y".equals(requestVo.getaEXYLClause())) {//恶性肿瘤赴日医疗责任
				...
				// 保额
				coverage10.setSumInsured(jsonAmounts.getDouble("EXYL")*securityNumDou);
				// 应收保费
			...
			if ("Y".equals(requestVo.getaEXZZClause())) {//恶性肿瘤扩展质子重离子医疗责任
				...
				// 保额
				coverage11.setSumInsured(jsonAmounts.getDouble("EXZZ")*securityNumDou);
				// 应收保费
			...
			try {
				...
				rs = service.accidentQuote(accrequest,null, result,
					...
					if (quotePriceResponseDTO != null) {
						if (quotePriceResponseDTO.getProposalNo() != null
								&& !"".equals(quotePriceResponseDTO
										.getProposalNo())) {
							//log.info("######意健险试算成功，投保单已生成。ProposalNo:{}," + new Object[] { quotePriceResponseDTO.getProposalNo() });
							// 定义投保信息对象存储投保信息
							...
							StoreInsureInfo insureInfo1 = storeProductPlaceUtilService
									.saveInsureInfo(storeInsureInfo, requestVo,
											result, requestVo.getHandlerInfo());// 保存试算信息
							// 提交核保
							//AccidentUnderwritingService underwritService = new AccidentUnderwritingService();
