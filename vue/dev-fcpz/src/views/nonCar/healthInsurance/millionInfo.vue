      ...
      >
        <van-swipe-item>
          <div class="swipeImg">
            <img
              v-if="isHome=='person'"
              :src="millionPersonImg"
            />
            <img
              alt
              v-else-if="isHome=='home'"
              :src="millionHomeImg"
            />
            <img
              v-else
      ...
      <div class="card">
        <van-cell-group>
          <van-cell
            title="投保人"
            icon="user-o"
            :value="millionInfo.appliName || '未填写'"
          />
          <van-cell
            title="被保人"
            icon="manager-o"
            :value="millionInfo.insuredName || '未填写'"
          />
          <van-cell
            title="保费"
            icon="bill-o"
          ...
          <div class="table">
            ...
            <div class="tbody">
              ...
              <span>基础版</span>
              <span>{{ isHome=='fangai' ? '200万':millionInfo.formatSumAmount }}</span>
            </div>
          </div>
        </van-cell-group>
      </div>
      <div class="card">
        <van-cell-group>
          ...
          <van-cell
            title="附加责任"
            class="bold"
            v-if="infoJsonPrice.length"
          />

          <div v-if="infoJsonPrice.length">
            <van-cell
              :title="infoJsonPrice[0].name"
              :value="infoJsonPrice[0].subname"
              v-if="millionInfo.aKZEXClause == 'Y'"
            />
            <van-cell
              :title="infoJsonPrice[1].name"
              :value="infoJsonPrice[1].subname"
              v-if="millionInfo.aEXTZClause == 'Y'"
            />
            <van-cell
              :title="infoJsonPrice[2].name"
              :value="infoJsonPrice[2].subname"
              v-if="millionInfo.aEXYLClause == 'Y'"
            />
            <van-cell
              :title="infoJsonPrice[3].name"
              :value="infoJsonPrice[3].subname"
              v-if="millionInfo.aEXZZClause == 'Y'"
            />
          </div>
        </van-cell-group>
      </div>

      <millionStatic v-if="isHome!='fangai'" />
      <div
        v-else
        class="card"
...
export default {
  data () {
    return {
      ...
      medicalImg: require("@/assets/nonCar/healthInsurance/glBanner.png"),
      isScroll: false,
  ...
  methods: {
    ...
    enter () {
      ...
      if (this.millionInfo.routerName == "healthTellNew") {
  ...
  created () {
    this.millionInfo = this.$route.query;
    this.isSheBao = this.millionInfo.isSheBao;
    // 判断是否是家庭版
    // this.isHome =
    // this.millionInfo.formulaName.indexOf("个人版") > 0 ? false : true;
    if (this.millionInfo.formulaName.indexOf("防癌") != -1) {
      this.isHome = 'fangai';
    } if (this.millionInfo.formulaName.indexOf("个人") != -1) {
      this.isHome = 'person';
    } if (this.millionInfo.formulaName.indexOf("家庭") != -1) {
      this.isHome = 'home';
    }
    //TODO suncao 商业计划书制作
    if (!this.$isNull(this.millionInfo)) {
      if (!this.$isNull(this.millionInfo.accidentalInjuryInfos)) {
        this.accidentalInjuryInfos = JSON.parse(
          this.millionInfo.accidentalInjuryInfos
        );
      }
      this.millionInfo.accidentalInjuryInfos = this.accidentalInjuryInfos || [];
      this.businessMobile = this.millionInfo.businessMobile || "95590";
      this.businessName = this.millionInfo.businessName || "";
      this.storeCode = this.millionInfo.storeCode || "";
    }

    this.routerName == this.millionInfo.routerName || "";
    localStorage.setItem("storeCode", this.storeCode);
    if (!this.$isNull(this.millionInfo)) {
      localStorage.setItem("millionHomeData", JSON.stringify(this.millionInfo));
    }
    if (!this.$isNull(this.millionInfo.agentInfo)) {
      agentInfo = JSON.parse(this.millionInfo.agentInfo);
    } else {
      agentInfo = null;
    }
    this.millionInfo.agentInfo = agentInfo;

    let proCode = "FM50";
    if (this.accidentalInjuryInfos && this.accidentalInjuryInfos.length > 1) {
      proCode = "FM51";
      ...
      for (let pos = 0; pos < len; pos++) {
        insureName +=
          this.accidentalInjuryInfos[pos].familyName +
          (pos < len - 1 ? "、" : "");
      }
      this.millionInfo.insuredName = insureName;
    }
    if (this.millionInfo.formulaCode == 'WTI2050018' || this.millionInfo.formulaCode == 'WTI2050019') {
      proCode = "WTI01"
    }

    this.getQueryFormula({
      proCode,
      storeCode: localStorage.getItem("storeCode") || this.storeCode
    })
      .then(res => {
        this.queryData = this.millionQuery;
        if (
          JSON.parse(localStorage.getItem("millionHomeData")).isSheBao == "Y"
        ) {
          let formula = this.queryData.data.sfList[0];
          this.setFormula({ formula }); // 选择方案
          // this.setNewAgesJson({ premiumsJson: formula.premiumsJson })

        } else {
          let formula = this.queryData.data.sfList[1];
          this.setFormula({ formula, result: this.$route.query.result }); // 选择方案
          // this.setNewAgesJson({ premiumsJson: formula.premiumsJson })
  ...
  computed: {
    ...
    // },
    ...mapGetters("million", ["millionData", "millionQuery"])
  }
};
</script>
