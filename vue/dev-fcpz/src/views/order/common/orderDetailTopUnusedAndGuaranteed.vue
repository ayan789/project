/* zhangminjie 公共组件--订单详情中头部卡片--已失效和保障中 */
<template>
  <div class="orderDetailTopUnusedAndGuaranteed">
    <div class="order">
      <van-card class="orderInfo">
        <div slot="title">
          <div class="orderInfoTop">
            <span>{{ insuranceInfo.desc }}</span>
            <!-- <button class="guaranteed" v-if="orderStauts == 1">
              保障中
            </button>
            <button class="unused" v-if="orderStauts == 2">
              已失效
            </button>
            <button
              class="guaranteed"
              v-if="orderStauts == 3 && insuranceType == 1"
            >
              待支付
            </button>-->
          </div>
        ...
        <!-- <div slot="footer">
          <div class="orderInfoBottom" v-if="orderStauts == 1">
            <button class="copyOrder" @click="copyInsurance">复制投保单</button>
            <button class="telInvoice" @click="eleTicket">电子发票</button>
            <button class="telInsurance" @click="eleInsurance">电子保单</button>
          </div>
          <div class="orderInfoBottom" v-if="orderStauts == 2">
            <button class="telInvoice" @click="copyInsurance">
              复制投保单
            </button>
            <button class="telInsurance" @click="deleteOrder">删除订单</button>
          </div>
          <div
            class="orderInfoBottom"
            v-if="orderStauts == 3 && insuranceType == 1"
          >
            <button class="telInvoice" @click="paymentMethod">
              立即支付
            </button>
            <button class="telInsurance" @click="uploadImg">影像上传</button>
          </div>
        </div>-->
      </van-card>
    </div>
...
Vue.use(Card);
export default {
  name: "orderDetailTopUnusedAndGuaranteed",
  props: {
